...
static void my_master(std::vector<std::string> args) {
    ...
    auto job_queue = std::make_shared<ConcurrencyQueue>("./config/test_input-c");
    ...
    //     std::make_shared<OnlineLearningModule>(master_manager, job_queue, 1, worker_host_num - 1, 10);
